...
export default function SMSTransactionsScreen({ navigation }) {
    ...
    const filterBankTransactions = async (messages) => {
        ...
        for (const msg of messages) {
            ...
            const exists = existingTransactions.some(t => 
                Math.abs(t.timestamp - transactionDate) < 1000 && 
                t.amount === amount && 
                t.bankName === bankName
            );
    ...
    const saveToFirebase = async (transaction) => {
        try {
            const q = query(
                collection(db, 'transactions'),
        ...
        <View style={styles.container}>
            ...
            {loading ? (
                ...
                <EmptyState
                    ...
                    message="Your bank transactions will appear here automatically"
                    actionText="Sync Now"
                    onAction={onRefresh}
                />
            )}
...
const styles = StyleSheet.create({
    ...
    header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: theme.spacing.md, paddingVertical: theme.spacing.md },
    headerTitle: { fontSize: theme.fontSize['2xl'], fontWeight: 'bold', color: theme.colors.text_primary },
    summaryCards: { flexDirection: 'row', paddingHorizontal: theme.spacing.md, gap: theme.spacing.sm, marginBottom: theme.spacing.md },
    summaryCard: { flex: 1, padding: theme.spacing.md, borderRadius: theme.borderRadius.lg, alignItems: 'center' },
    summaryLabel: { fontSize: theme.fontSize.sm, color: theme.colors.text_secondary, marginBottom: 4 },
